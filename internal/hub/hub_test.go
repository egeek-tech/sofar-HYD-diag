...
func TestSkipOverlappingReadCycle(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		// Wait long enough for all pending reads to complete (each has 200ms batchDelay,
		// and grid section has 7 spans). Under synctest virtual time, this is instant.
		time.Sleep(5 * time.Second)
		drainClientMessages(send, 200*time.Millisecond)

		// Under synctest virtual time, reads complete faster so more queued
		// read_cycles may execute. The invariant is that at least some are
		// skipped (not all 5 fire). Each read cycle involves multiple batch
		// calls (one per span), so the raw count reflects spans, not cycles.
		got := mb.getBatchCallCount()
		assert.Less(t, got, 5*7, "expected at least some overlapping read_cycles to be skipped")

		// Explicit cancel + sleep to let hub goroutine exit before bubble closes
		cancel()
		time.Sleep(50 * time.Millisecond)
	})
}
